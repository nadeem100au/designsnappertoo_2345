...
export function AnnotationDashboard({ onNavigate, data }: AnnotationDashboardProps) {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'contrast': return <Contrast className="w-3 h-3" />;
  ...
  const getTypeBadgeStyles = (type: string) => {
    switch (type) {
      case 'contrast': return 'bg-purple-100 text-purple-700 border-purple-200';
      ...
      <div className="flex h-[calc(100vh-73px)]">
        ...
        <Resizable defaultSize={{ width: 384, height: '100%' }} minWidth={320} maxWidth={800} enable={{ left: true }} className="border-l border-slate-200 bg-background shadow-2xl z-40 flex flex-col relative" onClick={(e) => e.stopPropagation()}>
          <div className="flex border-b">
            <button onClick={() => setSidebarMode('technical')} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest ${sidebarMode === 'technical' ? 'text-[#0066ff] border-b-2 border-[#0066ff]' : 'text-muted-foreground'}`}>Technical</button>
            {data.influencerReview && <button onClick={() => setSidebarMode('influencer')} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest ${sidebarMode === 'influencer' ? 'text-[#0066ff] border-b-2 border-[#0066ff]' : 'text-muted-foreground'}`}>Expert</button>}
          </div>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="pt-2 border-t border-slate-100/50 mt-2 flex justify-end">
                                <button 
                                  ...
                                  className="text-[12px] text-[#0066ff] hover:text-[#0052cc] transition-colors flex items-center gap-1 group/btn font-[Inter] font-bold"
                                >
                                  learn more
...
const getPsychologyInfo = (type: string) => {
  switch (type) {
    case 'contrast':
      return "Users with visual impairments rely on high contrast for legibility. Violating these standards causes immediate cognitive strain and exclusion.";
    case 'usability':
